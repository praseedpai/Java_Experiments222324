...
{
          T_FILE_META ret_val;
          memset(&ret_val,0,sizeof(T_FILE_META));  ret_val.packet_type = CHISTA_META_DATA;
          strcpy(ret_val.file_name, file_name);  ret_val.size = size;
          return ret_val;
}
...
T_HAND_SHAKE  MakeHandShake( ) {
         T_HAND_SHAKE ret_val;
          memset(&ret_val,0,sizeof(T_HAND_SHAKE));  ret_val.packet_type = CHISTA_HAND_SHAKE; strcpy(ret_val.hello_str,"hello");
          return ret_val;
}
...
T_ACK  MakeAck( ) {
         T_ACK  ret_val;
          memset(&ret_val,0,sizeof(T_ACK));  ret_val.packet_type = CHISTA_ACK; strcpy(ret_val.hello_str,"olleh");
          return ret_val;
}
...
T_FILE_EOF  MakeEof( ) {
         T_FILE_EOF ret_val;
          memset(&ret_val,0,sizeof(T_FILE_EOF));  ret_val.packet_type = CHISTA_META_EOF; strcpy(ret_val.eof_str,"EOF");
          return ret_val;
}
...
T_FILE_FATAL_ERROR MakeErrorPack( char * buffer , int len ) {
          ...
          else { strcpy(ret_val.err_msg,"Error Message Too Long..."); }
          return ret_val;
